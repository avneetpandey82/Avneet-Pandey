package com.foodShala.lib.controller;

import java.io.IOException;
import javax.servlet.ServletException;
import javax.servlet.annotation.WebServlet;
import javax.servlet.http.HttpServlet;
import javax.servlet.http.HttpServletRequest;
import javax.servlet.http.HttpServletResponse;

import com.foodShala.lib.dao.Dao;
import com.foodShala.lib.dao.DaoImp;

@WebServlet("/RestMenuDelete")
public class RestMenuDelete extends HttpServlet {
	Dao dao;
	public RestMenuDelete() {
		dao = new DaoImp();
	}
	
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		int id = Integer.parseInt(request.getParameter("id"));
		System.out.println(id);
		boolean status = dao.delete(id);
		request.getRequestDispatcher("RestMenuDisplay").forward(request, response);
//		RestMenuDisplay rmd = new RestMenuDisplay();
//		
//		if(status)
//			rmd.doGet(request, response);
//		else
//			rmd.doGet(request, response);
		
	}
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		doGet(request, response);
	}

}
